...
import { Player, Course, Tournament, Registration } from "@/lib/googleSheets";
...
export default function Dashboard() {
    ...
    useEffect(() => {
        if (!authLoading) {
            ...
            // Redirect to profile setup if not admin and no member profile
            // (Admins might not need a player profile, but regular users do)
                                    ...
                                    {dataLoading ? (
                                        ...
                                        recentRegistrations.map((reg) => (
                                            <div key={reg.id} className="p-5 flex items-center justify-between hover:bg-gray-50 transition-colors">
                                                ...
                                                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${reg.status === 'paid'
