...
export default function TournamentManagePage({ params }: PageProps) {
    ...
    const fetchData = async () => {
        try {
            const [tournamentsRes, playersRes, scoresRes, coursesRes] = await Promise.all([
            ...
            // Determine active players based on scores
            const playerIdsWithScores = new Set(scoresData.map(s => s.playerId));
            const initialActivePlayers = validPlayers.filter(p => playerIdsWithScores.has(p.id));
